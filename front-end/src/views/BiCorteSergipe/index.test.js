...
describe("BiCorteSergipeView", () => {
  ...
  it("preenche a sugestao, envia ao n8n e exibe a resposta", async () => {
    render(<BiCorteSergipeView />);
    fireEvent.click(
      screen.getByRole("button", {
        name: "Quantas OS concluídas hoje?",
      }),
    );

    expect(screen.getByLabelText("Digite sua pergunta")).toHaveValue(
      "Quantas OS concluídas hoje?",
    );
    ...
    expect(payload.message).toBe("Quantas OS concluídas hoje?");
